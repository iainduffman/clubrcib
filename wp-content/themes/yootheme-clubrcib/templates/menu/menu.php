...
if (strpos($position, 'navbar') === 0) {
    ...
    if (preg_match('/^(offcanvas|modal)/', $layout)) {
        ...
        $type = 'navbar';
        $attrs['class'][] = 'uk-navbar-nav primary-nav';
...
<ul<?= $this->attrs($attrs) ?>>
    ...
    <a href="http://deveurorescue.azurewebsites.net/join.aspx?coverType=1" target="_blank" class="uk-button uk-button-default tm-button-default uk-icon uk-margin-left">Join Today</a>
</ul>
